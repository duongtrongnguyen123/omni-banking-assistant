...
def forecast(user_id: str, when: datetime) -> Optional[dict]:
    """Project month-end spend & remaining balance based on the running rate.

    Returns ``None`` when there's nothing reliable to forecast — early in
    the month (≤3 days elapsed) the daily rate is dominated by single
    transactions and projection becomes noise. We also bail when this
    month has zero transactions yet.

    Output:
      - ``days_elapsed``      / ``days_in_month``
      - ``spent_so_far``      (this month, integer VND)
      - ``daily_rate``        (spent_so_far / days_elapsed, integer VND)
      - ``projected_total``   (daily_rate × days_in_month)
      - ``projected_remaining_spend`` (projected_total − spent_so_far)
      ...
      - ``pace_vs_last_month`` ratio of projected_total / last_month_total
                               (None when last_month_total == 0)
    """
    from ..store import get_store
