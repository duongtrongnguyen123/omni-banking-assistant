...
USER = "u_an"
SAVINGS_ACCOUNT = "acc_an_savings"
...
def test_normal_transfer_requires_otp() -> None:
    print_step("1. Normal transfer -> OTP -> executed")
    reset_session()
    ...
    assert_true(draft.auth_required == ["otp"], f"Expected OTP only, got {draft.auth_required}")
    ...
    confirmed = confirm_draft(USER, draft.id, otp="123456")
...
def test_large_transfer_blocks_primary_account() -> object:
    ...
    print_step("3. Large transfer on primary account -> BLOCK")
    reset_session()
    ...
    assert_true("insufficient_balance" in flag_codes(resp), f"Expected insufficient_balance, got {flag_codes(resp)}")
    assert_true(draft.auth_required == [], f"Blocked draft should not expose auth path, got {draft.auth_required}")
...
def test_savings_account_needs_otp_and_biometric(draft) -> None:
    print_step("4. Switch to savings -> OTP + biometric -> executed")

    otp_resp = confirm_draft(
        USER,
        draft.id,
        otp="123456",
        source_account_id=SAVINGS_ACCOUNT,
    )
    next_draft = otp_resp.draft

    assert_true(next_draft is not None, "Expected draft to remain until biometric passes")
    assert_true(
        next_draft.auth_required == ["otp", "biometric"],
        f"Expected OTP + biometric, got {next_draft.auth_required}",
    )
    assert_true(
        next_draft.auth_completed == ["otp"],
        f"Expected OTP completed, got {next_draft.auth_completed}",
    )
    print("after OTP:", otp_resp.text)
    print("auth_required:", next_draft.auth_required)
    print("auth_completed:", next_draft.auth_completed)

    bio_resp = confirm_draft(USER, draft.id, biometric_verified=True)
    assert_true(bio_resp.draft is None, "Expected transfer to execute after biometric")
    assert_true("Da chuyen" in _fold_text(bio_resp.text), bio_resp.text)
